...
double generate_spot_prices(int num_particles, int num_weeks, double strike_price , double time_maturity,double spot_price, double risk_free_rate, double volatility){
    ...
    double C = 0.0;
    vector<double> spot_prices(num_particles*(num_weeks + 1));
    ...
    for (int t = 0; t < num_particles; t++) {
        ...
        spot_prices[t*num_weeks] = spot_price;

        // Calculate the spot price at the current time step.
        #pragma omp critical
        for (int i = 0; i < num_weeks; i++) {
            spot_prices[t*num_weeks+i+1] = spot_prices[t*num_weeks+i] * exp(nudt + sidt * dist(gen));
        }
        C += max(spot_prices[t*num_weeks+num_weeks-1] - strike_price, 0.0);
    }       
    }    
